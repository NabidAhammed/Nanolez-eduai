...
const App = () => {
  ...
  useEffect(() => {
    if (!user) return;
    const roadmapsRef = collection(db, 'artifacts', appId, 'public', 'data', 'roadmaps');
    const unsubscribe = onSnapshot(roadmapsRef, (snapshot) => {
      ...
      setRoadmaps(data);
      if (data.length > 0 && !activeRoadmapId) setActiveRoadmapId(data[0].id);
    }, (err) => setError("Cloud sync error."));
    return () => unsubscribe();
  }, [user]);
  ...
  const generateRoadmap = async () => {
    ...
    const system = `JSON only: { "title": "...", "months": [{ "name": "...", "weeks": [{ "name": "...", "days": [{ "day": 1, "task": "...", "topic": "..." }], "weeklyProject": "..." }] }] }`;
    try {
      const result = await fetchGemini(prompt, system);
      const data = JSON.parse(result.candidates[0].content.parts[0].text);
      const newRef = doc(collection(db, 'artifacts', appId, 'public', 'data', 'roadmaps'));
      await setDoc(newRef, { 
        ...data, 
        userId: user.uid, 
        goal: formData.goal, 
  ...
  const submitQuiz = async () => {
    let earned = 0;
    quizModal.questions.forEach((q, i) => { if (quizModal.userAnswers[i] === q.correctIndex) earned++; });
    const scoreKey = quizModal.type === 'final' ? 'final' : quizModal.weekName;
    const nextScores = { ...(activeRoadmap.scores || {}), [scoreKey]: earned };
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'roadmaps', activeRoadmapId), { scores: nextScores });
    setQuizModal(p => ({ ...p, score: earned }));
  };

  const toggleTask = async (roadmapId, key) => {
    const r = roadmaps.find(it => it.id === roadmapId);
    if (!r) return;
    const next = { ...(r.progress || {}), [key]: !r.progress?.[key] };
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'roadmaps', roadmapId), { progress: next });
  };
  ...
  const deleteRoadmap = async (id, e) => {
    ...
    if (confirm("Delete this roadmap?")) {
      try {
        await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'roadmaps', id));
                          ...
                          <div key={wIdx} className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {week.days?.map(d => {
                                ...
                                return (
                                  <div key={d.day} className={`flex border rounded-lg overflow-hidden group transition-all ${done ? 'bg-slate-50 border-slate-100 opacity-75' : 'bg-white border-slate-200 hover:border-indigo-400 hover:shadow-md'}`}>
                                    <div onClick={() => toggleTask(activeRoadmap.id, key)} className="p-3 flex gap-3 cursor-pointer flex-1 items-center">
                                      <div className={`transition-all ${done ? 'text-emerald-500' : 'text-slate-200 group-hover:text-indigo-500'}`}>
                                        {done ? <CheckCircle2 className="w-5 h-5" /> : <Circle className="w-5 h-5" strokeWidth={2} />}
                                      </div>
                                      <div className="min-w-0">
                                        <p className="text-[7px] font-black text-slate-400 uppercase tracking-[0.15em] mb-0.5">Day {d.day} • {cleanText(d.topic)}</p>
                                        <p className={`text-[10px] font-bold truncate ${done ? 'line-through text-slate-400' : 'text-slate-700'}`}>{cleanText(d.task)}</p>
                                      </div>
                                    </div>
                                    <button onClick={() => fetchAIArticle(d)} className="p-3 border-l border-slate-100 flex items-center justify-center hover:bg-indigo-50 transition-all text-indigo-600 group/btn bg-slate-50/50">
                                       <Newspaper className="w-3.5 h-3.5 group-hover/btn:scale-110" />
                                    </button>
                                  </div>
                                );
                              })}
